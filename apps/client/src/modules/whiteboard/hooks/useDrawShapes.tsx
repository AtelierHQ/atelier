...
function useDrawShapes(
  ...
  const [selectedShapeConfig, setSelectedShapeConfig] = useState<SelectedShape<ShapesEnum>>();
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    if (isDrawing && context && toDraw) {
      ...
      drawExistingShapes(shapesMap, canvasConfig);

      if (toDraw === ShapesEnum.Pen) {
        penCoordinates.current.push({ x: event.offsetX, y: event.offsetY });
        context.beginPath();
        context.moveTo(penCoordinates.current[0].x, penCoordinates.current[0].y);
        ...
        drawShapes(context, toDraw, event, initCoord, shapeId.current, activeShape, canvasRef);
      }
    }
    ...
    if (!isDrawing && !toDraw) {
      if (selectedShapeConfig?.isActive && context) {
        ...
        drawExistingShapes(shapesMap, canvasConfig, selectedShapeConfig.id);

        activeShape.current = moveShape(selectedShapeConfig, event, context, once);
  ...
  function handleMouseUp() {
    ...
    if (context && isDrawing && activeShape.current) {
      ...
      activeShape.current &&
        setShapesMap(shapesMap.set(selectedShapeConfig.id, activeShape.current));
  ...
  function handleDrawText(event: MouseEvent, shapeId: string) {
    const { typingArea, x1, y1 } = getTypingArea(event, canvasRef);
    ...
    switch (shape.type) {
      case ShapesEnum.Rectangle: {
        if (isRectangle(shape)) {
          ...
          if (
            currentX === x1 ||
            (currentX >= x1 - 10 && currentX <= x1 + 10 && currentY > y1 && currentY < y2)
          ) {
